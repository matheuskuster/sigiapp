...
class CommitteController {
  ...
  async generateUsers(req, res) {
    ...
    // console.log(committe.delegations)
  ...
  async showUsers(req, res) {
    ...
    // Promise.all(delegates).then(async completed => {
    //   const sheet = await SheetController.createSheet(committe, completed)
    // })

    const sheet = await SheetController.createSheet(committe, delegates)

    return res.render('users', { delegates, sheet, committe })
  }
  ...
  async controlPanel(req, res) {
    ...
    const dele = await User.find({
      ...
      .select(['_id', 'present', 'delegation', 'presency'])
      .populate('delegation')
    ...
    //   var delegate = await User.findById(delegates[i].id)

    //   if (delegate.present > 0) {
    //     delegate.presency[committe.currentSession - 1] = true
    //   }
    //   else {
    //     delegate.presency[committe.currentSession - 1] = false
    //   }
    ...
    delegates.map(async delegate => {
      delegate.presency[committe.currentSession - 1] = delegate.present > 0

      await User.findByIdAndUpdate(delegate._id, {
        presency: delegate.presency
      })
    })
  ...
  async shutSession(req, res) {
    ...
    committe.currentSession = committe.currentSession + 1
  ...
  async project(req, res) {
    ...
    // console.log(delegates[0].presency)
